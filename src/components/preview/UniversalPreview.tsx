...
export function UniversalPreview({ doc }: UniversalPreviewProps) {
  ...
  const displayName = fixFilename(doc.name);
  ...
  return (
    <div className="lo-preview-panel lo-preview-panel--full">
      <div className="lo-preview-header">
        <span>{displayName}</span>
        {preview?.url && (
          <a href={preview.url} download={displayName} className="flex items-center gap-1 text-[11px] hover:underline">
      ...
      <div className="lo-preview-body">
        {loading ? (
          ...
          <div className="lo-pdf-frame">
            <PdfViewer ref={pdfRef} url={preview.url} fileName={displayName} />
          </div>
        ) : preview?.type === "image" && preview.url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={preview.url} alt={displayName} className="max-h-full max-w-full object-contain shadow-lg" />
          ...
          <div className="flex flex-col items-center gap-4 rounded bg-white p-8 shadow-lg">
            ...
            <div className="text-center">
              ...
              <p className="mt-3 max-w-sm text-xs text-gray-500">
                {preview?.message || "원본 다운로드 가능"}
              </p>
            </div>
            {preview?.url && (
              <a href={preview.url} download={displayName}
