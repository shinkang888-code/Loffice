  ...
  if (loading) {
    return (
      <div className="flex h-48 items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-loffice-teal border-t-transparent" />
      </div>
    );
  }

  if (error) {
    return <div className="flex h-48 items-center justify-center text-red-600">{error}</div>;
  }
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 bg-white px-4 py-2">
        ...
        <span className="min-w-[80px] text-center text-sm">{page} / {totalPages}</span>
        <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
          className="rounded p-1.5 hover:bg-gray-100 disabled:opacity-40">
          <ChevronRight className="h-5 w-5" />
        </button>
        <div className="mx-2 h-5 w-px bg-gray-200" />
        <button onClick={() => setScale((s) => Math.max(0.5, s - 0.2))} className="rounded p-1.5 hover:bg-gray-100">
          <ZoomOut className="h-5 w-5" />
        </button>
        <span className="min-w-[48px] text-center text-sm">{Math.round(scale * 100)}%</span>
        <button onClick={() => setScale((s) => Math.min(3, s + 0.2))} className="rounded p-1.5 hover:bg-gray-100">
